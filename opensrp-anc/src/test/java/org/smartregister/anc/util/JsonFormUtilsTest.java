...
public class JsonFormUtilsTest {
    ...
    public void testGetAutoPopulatedJsonEditFormStringInjectsValuesCorrectlyInForm() throws Exception {
        ...
        JsonFormUtils.getAutoPopulatedJsonEditRegisterFormString(context.applicationContext(), null);  //Exception Branch
        ...
        PowerMockito.when(locationHelper.generateDefaultLocationHierarchy(healthFacilities)).thenReturn(allLevels);
        PowerMockito.when(locationHelper.generateLocationHierarchyTree(ArgumentMatchers.anyBoolean(), ArgumentMatchers.eq(healthFacilities))).thenReturn(new ArrayList<FormLocation>());
